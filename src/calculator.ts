...
export function calcRSU102(saleData: SaleData, taxes: Taxes, grantPrice: number) : number {
    ...
    const netPerUnit = saleData.salePrice - incomeTaxPerUnit - capitalGainTaxPerUnit;

    return netPerUnit * saleData.totalAmount;
}

export function calcRSU(saleData: SaleData, taxes: Taxes) : number {
    const incomeTaxPerUnit = saleData.salePrice * (taxes.incomeTax + taxes.surTax);
    const netPerUnit = saleData.salePrice - incomeTaxPerUnit;

    return netPerUnit * saleData.totalAmount;
}

export function calcESOP102(saleData: SaleData, taxes: Taxes, grantPrice: number ) : number {
    const capitalGainTaxPerUnit =
        (saleData.salePrice - grantPrice) * (taxes.capitalGainTax + taxes.surTax);
